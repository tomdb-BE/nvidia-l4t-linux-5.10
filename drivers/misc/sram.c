...
/*
 * SYSRAM contains areas that are not accessible by the kernel, such as the
 * first 256K that is reserved for TZ. Accesses to those areas (including
 * speculative accesses) trigger SErrors, so only map the child regions
 * explicitly described in the device tree.
 */
...
static int sram_probe(struct platform_device *pdev)
{
	...
	sram->dev = &pdev->dev;
	sram->no_memory_wc = of_property_read_bool(pdev->dev.of_node,
					      "no-memory-wc");
	sram->config = config;
